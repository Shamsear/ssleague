        ...
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <ul className="text-sm text-blue-800 space-y-1">
            ...
            <div className={`p-6 rounded-lg border-2 ${
              currentPhase === 'closed' ? 'bg-gray-50 border-gray-300' :
              currentPhase === 'release' ? 'bg-purple-50 border-purple-300' :
              currentPhase === 'draft' ? 'bg-indigo-50 border-indigo-300' :
              'bg-cyan-50 border-cyan-300'
            }`}>
              <div className="flex items-center gap-3 mb-2">
                <div className={`w-3 h-3 rounded-full ${
                  currentPhase === 'closed' ? 'bg-gray-500' :
                  currentPhase === 'release' ? 'bg-purple-500 animate-pulse' :
                  currentPhase === 'draft' ? 'bg-indigo-500 animate-pulse' :
                  'bg-cyan-500 animate-pulse'
                }`}></div>
                <h4 className="text-xl font-bold text-gray-900">
                  {currentPhase === 'closed' ? 'All Windows Closed' :
                   currentPhase === 'release' ? 'Release Phase Active' :
                   currentPhase === 'draft' ? 'Draft Phase Active' :
                   'Trading Phase Active'}
                </h4>
              </div>
              <p className="text-gray-700">
                {currentPhase === 'closed' ? 'No transfer activity is currently allowed.' :
                 currentPhase === 'release' ? 'Teams can release players for 80% budget refund.' :
                 currentPhase === 'draft' ? 'Teams can bid on released players.' :
                 'Teams can propose direct trades with each other.'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
              <h4 className="font-semibold text-purple-900 mb-2">Release Phase</h4>
              <p className="text-sm text-purple-700 mb-3">48 hours, 80% refund</p>
              <button 
                onClick={() => handleOpenPhase('release')}
            ...
            <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-200">
              <h4 className="font-semibold text-indigo-900 mb-2">Draft Phase</h4>
              <p className="text-sm text-indigo-700 mb-3">48 hours, tier-based</p>
              <button 
                onClick={() => handleOpenPhase('draft')}
            ...
            <div className="bg-cyan-50 rounded-lg p-4 border border-cyan-200">
              <h4 className="font-semibold text-cyan-900 mb-2">Trading Phase</h4>
              <p className="text-sm text-cyan-700 mb-3">Ongoing, 48h expiry</p>
              <button 
                onClick={() => handleOpenPhase('trading')}
                className="w-full px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 transition-colors text-sm font-medium"
              >
                Enable Trading
              </button>
            </div>
