...
export default function PublicAwardsPage() {
  ...
  const cleanedPlayerAwards = Array.isArray(playerAwards) ? playerAwards.filter(award => {
    ...
    // Skip short-term awards that are already shown in the detailed 'awards' section
  ...
  // Calculate counts respecting the selected season
        ...
        <header className="mb-6 sm:mb-8 text-center lg:text-left">
          ...
          <div className="mt-4 sm:mt-6 flex flex-wrap gap-2 sm:gap-3 justify-center lg:justify-start">
            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-white/80 backdrop-blur-sm rounded-full border border-gray-200 shadow-sm">
              ...
              <span className="text-xs sm:text-sm font-semibold text-gray-900">{filteredAwardsCount} Awards</span>
            </div>
            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-white/80 backdrop-blur-sm rounded-full border border-gray-200 shadow-sm">
              <span className="text-lg sm:text-xl">🏆</span>
              <span className="text-xs sm:text-sm font-semibold text-gray-900">{filteredTrophiesCount} Trophies</span>
            </div>
          </div>
        </header>
        ...
        {!loading && filteredData.items.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-5 mb-8 sm:mb-12">
            ...
            {activeTab === 'awards' && Array.isArray(awards) && awards
              .filter(award => selectedSeason === 'all' || award.season_id === selectedSeason)
              .map((award, index) => {
            ...
            {activeTab === 'awards' && Array.isArray(cleanedPlayerAwards) && cleanedPlayerAwards
              .filter(award => selectedSeason === 'all' || award.season_id === selectedSeason)
              .map((award, index) => {
            ...
            {/* Trophies */}
            {activeTab === 'trophies' && Array.isArray(trophies) && trophies
              .filter(trophy => selectedSeason === 'all' || trophy.season_id === selectedSeason)
              .map((trophy, index) => {
